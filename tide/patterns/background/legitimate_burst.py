...
class LegitimateBurstStructural(StructuralComponent):
    ...
    def select_entities(self, available_entities: List[str]) -> EntitySelection:
        ...
        # Config-driven burst fraction
        burst_cfg = self.params.get("backgroundPatterns", {}).get(
            "legitimateBurst", {}
        )
        burst_fraction = burst_cfg.get("burst_user_fraction", 0.05)
...
class LegitimateBurstTemporal(TemporalComponent):
    ...
    def _generate_transactions_stream(self, entity_selection: EntitySelection):
        ...
        # Config-driven burst size and amount
        burst_cfg = self.params.get("backgroundPatterns", {}).get(
            "legitimateBurst", {}
        )
        burst_size_range = burst_cfg.get("burst_size_range", [3, 6])
        amount_range = burst_cfg.get("amount_range", [50.0, 500.0])
        ...
        for user in burst_users:
            ...
            burst_start_time = start_date + datetime.timedelta(
                days=burst_day, hours=random_instance.randint(10, 20)
            )

            # Generate burst_size_range[0]-burst_size_range[1] transactions within ~1 hour
            ...
            for _ in range(num_txs):
                ...
                # Very short time intervals (1-60 mins from start)
                time_offset = random_instance.randint(1, 60)
                tx_time = burst_start_time + \
                    datetime.timedelta(minutes=time_offset)

                # Pick a random legitimate counterparty; avoid self-transfers
                target = random_instance.choice(targets)
                if target == user:
                    continue

                # Amount range intentionally overlaps with typical fraud ranges
                amount = round(
